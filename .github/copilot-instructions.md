...
- **Russian, Ukrainian and Belarusian are the exception: use the FORMAL/polite address** — Russian вы (not ты), Ukrainian ви (not ти), Belarusian вы (not ты), with the matching verb forms. This overrides the informal rule above for those three languages only. `translate_md.py` requests `formality=prefer_more` for them and `prefer_less` for everyone else; `register_ok()` checks the result against the language's expected register.
