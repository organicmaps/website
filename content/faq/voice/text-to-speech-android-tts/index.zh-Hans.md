...
- 选择其他设置，然后选择辅助功能
- 选择你喜欢的引擎、语速和音调
- **重新启动 Organic Maps 应用程序**
- 打开 Organic Maps 中的设置 => 语音指令并进行设置
- 如果语音不起作用，请再次重新启动 Organic Maps 应用程序（或重新启动设备）

如果找不到相关设置，请打开设置应用程序并搜索文本转语音。

P.S：请注意，这些步骤会根据你使用的手机品牌而有所不同。

如果你的设备上尚未安装 TTS，则可能不会显示上述选项。请参阅下表安装其中任何一个支持你的母语的软件。
...
如果你在 LineageOS 或其他自定义 ROM 上初始化 RHVoice TTS 引擎时遇到问题，请尝试此解决方法。 RHVoice 可能无法正确初始化，并且应用程序可能会崩溃，特别是如果你之前没有在手机上使用过任何 TTS 引擎（例如，新安装、恢复出厂设置等）。如果你使用的是 LineageOS 等自定义 ROM，<ins>没有 Google Play 服务和 Google 语音服务</ins>，并且希望使用 RHVoice 作为首选 TTS 引擎，请按照以下说明作为解决方法：
...
4. 安装 F-droid 上可用的 [RHVoice](https://f-droid.org/en/packages/com.github.olga_yakovleva.rhvoice.android/)。
    - 打开它，选择你要使用的语言，点击云图标（最左侧）下载语音。
    - 按播放按钮验证其是否正常工作
5. 将 **RHVoice** 设置为首选引擎（参见步骤 2）
6. 现在，你应该可以毫无问题地使用 RHVoice

## 测试

为了测试语音指令，你可以点击 OM“设置 → 语音指令”菜单中的“测试语音指令（TTS、文本转语音）”，或者你可以实际启动导航以接收任何语音输出。当你站立不动时，Organic Maps 不会给你任何语音指示。
